...
static {
	...
	// 得到logger实例，作为输出工具。
	// 此句作用是用一个输出实例，取代每个类里面的： LoggerFactory.getLogger(X.class)
	//logger = org.apache.log4j.Logger.getLogger("Logs");
	logger = LoggerFactory.getLogger("Logs");
}
...
private static void log(int level, Object message, StackTraceElement[] ste) {
   if (ste != null) {
    // 加入源代码定位
    message = msgSplit + message + " [in "+getStackMsg(ste)+"]";
   }
...
private static String getStackMsg(StackTraceElement[] ste) {
   ...
   for (int i = 0; i < ste.length; i++) {
    ...
    if (srcFlag) {
    	...
    	return s==null?"":s.getClassName()+"."+s.getMethodName();
    }
...
public static void error2(Exception e) {
	...
	log(error, sw.toString(), Thread.currentThread().getStackTrace());
}
}
